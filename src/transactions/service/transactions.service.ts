...
export class TransactionsService {
  constructor(private readonly paymentProvider: PagarMeProvider) {}
  ...
  async process(
    ...
    const transaction = { id: '', total: 0, installments: 0 };

    const items = null;

    const date = new Date();
    date.setDate(date.getDate() + 3); //3 dias úteis
    const boleto_expiration_date = `${date.getFullYear()}/${date.getMonth() +
      1}/${date.getDate()}`;
